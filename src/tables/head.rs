...
impl Head {
    ...
    pub(crate) fn from_reader<B: Read + Seek>(reader: &mut VeroBufReader<B>, metadata: &TableMetadata) -> Result<Self, VeroTypeError>{
        ...
        Ok(Self {
            ...
            checksum_adjustment: u32::from_be_bytes(buf[8..12].try_into().unwrap()),
            magic_number: u32::from_be_bytes(buf[12..16].try_into().unwrap()),
            flags: HeadFlags::from_bits(u16::from_be_bytes(buf[16..18].try_into().unwrap())),
            units_per_em: u16::from_be_bytes(buf[18..20].try_into().unwrap()),
            created: i64::from_be_bytes(buf[20..28].try_into().unwrap()),
            modified: i64::from_be_bytes(buf[28..36].try_into().unwrap()),
            x_min: i16::from_be_bytes(buf[36..38].try_into().unwrap()),
            y_min: i16::from_be_bytes(buf[38..40].try_into().unwrap()),
            x_max: i16::from_be_bytes(buf[40..42].try_into().unwrap()),
            y_max: i16::from_be_bytes(buf[42..44].try_into().unwrap()),
            mac_style: u16::from_be_bytes(buf[44..46].try_into().unwrap()),
